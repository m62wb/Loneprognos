...
function getOB3Periods(year) {
  ...
  // Nationaldag (FlexHRM-anpassad: om 6/6 är lördag startar OB3 på fredagen)
...
// ---- Huvudfunktion för OB (med intervallbaserad blandning) ----
function calcOB(date, shift, lag) {
  ...
  // 3) OB3 – subtrahera bara den del av normal OB som överlappar OB3-perioden
  ...
  if (ob3 > 0) {
    ...
    // Subtrahera OB3-överlapp från respektive intervall
...
function getOBForMonth(year, month, lag) {
  ...
  for (let d = 1; d <= dim; d++) {
    let date = new Date(year, month - 1, d), key = localDateKey(date);
